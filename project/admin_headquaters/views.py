...
from project.models import Admin_District,Admin_Headquaters,Complaints,Rejected_Complaint,Bar,User,ExportApproved
from flask_restful import Resource, Api
from flask import flash, redirect, render_template, request, url_for,make_response
from flask_login import login_user,login_required, logout_user
from project.models import Admin_District,Complaints,Rejected_Complaint,Bar,ComplaintStore,User,ExportApproved
...
class Register2(Resource):
    ...
    def post(self):
        ...
        unique_account=Admin_Headquaters.query.filter_by(publicID=data['public_id']).first()
        if unique_account is not None:
            admin_Name= data['Name']
            admin_email = data['email']
            publicID =unique_account.publicID
            ...
            if password==confirm_password:
                user = Admin_Headquaters(admin_Name=admin_Name,admin_email=admin_email,password=password,publicID=publicID,role=role)
                db.session.add(user)
                db.session.commit()
                return data
            else:
    ...
    def post(self):
        ...
        if admin.password == data['password']:
            ...
            if admin.role=="USER":
                ...
                activity=ExportApproved(name=name,date=date)
                db.session.add(activity)
                db.session.commit()
            else:
                pass
            return jsonify({'token':token.decode('UTF-8'),'username':admin.admin_email,'admin_Name':admin.admin_Name,'role':admin.role,'public_id':admin.publicID})
...
class UserActivity(Resource):
    ...
    def post(self,current_user):
        data=request.get_json()
        complaint=ExportApproved.query.all()
        return [x.json() for x in complaint[-1::-1]]
    ...
    def post(self,current_user):
        ...
        if complaint is not None:
            ...
            if status =="Resolved":
                ...
                insert = Rejected_Complaint(agent_name=agent_name,agent_email=agent_email,complainant_phone=complainant_phone,agent_staff=agent_staff,
                                            agent_idn0=agent_idn0,agent_post=agent_post,district=district,complainant_location=complainant_location,
    ...
    def post(self,current_user):
        ...
        if data['complaints_refn0'] is not None:
            ...
            if data['status'] is not None:
                ...
                headdescription = ''
                headclassification= ''
                update=Complaints(agent_name=agent_name,agent_email=agent_email,complainant_phone=complainant_phone,agent_idn0=agent_idn0,agent_staff=agent_post,district=district,
                                  complainant_location=complainant_location,complaint=complaint,
...
class PostzComplaint(Resource):
    ...
    def post(self,current_user):
       ...
       # while True:
        ...
        complainant_phone = data['complain_n0']
        agent_idn0 = ''
        agent_staff = data['agent_staff']
        district = data['district']
        complainant_location = data['poling_station']
        ...
        complaint = Complaints(agent_email=agent_email,agent_name=agent_name,complainant_phone=complainant_phone,agent_idn0=agent_idn0,agent_staff=agent_staff,compalianant_name=compalianant_name,
                                district=district,complainant_location=complainant_location,complaints_refn0=complaints_refn0,nature_complaint=nature_complaint,Complaint_category=Complaint_category,
    ...
    def post(self,current_user):
        ...
        for x in complaint:
            if x.headagent_name:
                ...
                agent_phone= x.complainant_phone
                agent_idn0= x.agent_idn0
                agent_post= x.agent_staff
                district= x.district
                poling_station = x.complainant_location
...
class Accounts(Resource):
    ...
    def post(self,current_user):
        ...
        try:
            ...
            return [x.json() for x in accounts]
        except Exception:
...
class DB_registration(Resource):
    @token_required
    def post(self,current_user):
        data = request.get_json()
        ...
        user = User(first_name=first_name,last_name=last_name,login=login,email=email,password=new_pass)
        db.session.add(user)
        db.session.commit()

class Getdbtable(Resource):
    @token_required
    def post(self,current_user):
        data = request.get_json()
        new_query=User.query.all()
        return [x.json() for x in new_query]
